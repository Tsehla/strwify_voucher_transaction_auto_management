...
-----------
PS this app uses old nodejs engine, so newwer features like [ replaceAll ] may not work on heroku since now you have to specifiy nodejs version which i used old one when i was developing the app which is node v12. lot of modules breaks on windows enviroment, so use ubuntu to avoid lot of issues when developing or hosting
